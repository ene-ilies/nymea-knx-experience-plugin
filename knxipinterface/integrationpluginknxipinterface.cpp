...
void IntegrationPluginKnxIPInterface::startMonitoringAutoThings()
{
    // Start seaching for devices which can be discovered and added automatically
}

void IntegrationPluginKnxIPInterface::discoverThings(ThingDiscoveryInfo *info)
{
    if (info->thingClassId() == KNXIPGatewayThingClassId) {
        ...
        connect(m_discovery, &KnxServerDiscovery::discoveryFinished, info, [this, info]() {
            qCDebug(dcKNXIP()) << "Discovery finished.";
            foreach (const QKnxNetIpServerInfo &serverInfo, m_discovery->discoveredServers()) {
                qCDebug(dcKNXIP()) << "Found server:" << QString("%1:%2").arg(serverInfo.controlEndpointAddress().toString()).arg(serverInfo.controlEndpointPort());
                KnxServerDiscovery::printServerInfo(serverInfo);
                ThingDescriptor descriptor(KNXIPGatewayThingClassId, "KNXIP Gateway", QString("%1:%2").arg(serverInfo.controlEndpointAddress().toString()).arg(serverInfo.controlEndpointPort()));
                ParamList params;
...
void IntegrationPluginKnxIPInterface::setupThing(ThingSetupInfo *info)
{
    qCDebug(dcKNXIP()) << "Setup device" << info->thing()->name() << info->thing()->params();
...
{
    qCDebug(dcKNXIP()) << "Setting KNX tunnel";
    this->tunnel = knxTunnel;
}*/
...
void IntegrationPluginKnxIPInterface::executeAction(ThingActionInfo *info)
{
...
void IntegrationPluginKnxIPInterface::createGenericDevices(Thing *parentThing)
{
    Q_UNUSED(parentThing)
...
void IntegrationPluginKnxIPInterface::destroyGenericDevices(Thing *parentThing)
{
    Q_UNUSED(parentThing)
...
void IntegrationPluginKnxIPInterface::onPluginTimerTimeout()
{
...
void IntegrationPluginKnxIPInterface::onPluginConfigurationChanged(const ParamTypeId &paramTypeId, const QVariant &value)
{
...
void IntegrationPluginKnxIPInterface::onTunnelFrameReceived(const QKnxLinkLayerFrame &frame)
{
    Q_UNUSED(frame);
    /*foreach (Thing *thing, myThings()) {
